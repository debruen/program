...
std::vector<double> Specone::fft(std::vector<double> input) {
...
void Specone::norm3dDouble(std::vector< std::vector< std::vector<double> > >& vector) {
...
nlohmann::json Specone::data() {

  return specone_data;
} // data() END

nlohmann::json Specone::data(nlohmann::json data) {
...
void Specone::process(std::vector<cv::Mat>& images, stk::StkFrames& audio) {

  std::string chan_a, chan_b, chan_c;

  chan_a = data::get_string(specone_data, "a");
  chan_b = data::get_string(specone_data, "b");
  chan_c = data::get_string(specone_data, "c");
  ...
  double freq, keys, base, narrowing, table_specone[256], table_lr[sine_bands][2], x_correction;

  narrowing = specone_data[1]["value"];

  if(specone_data[2]["value"] == 88) {
    freq = 27.5000;
    keys = 76.0;
  } else {
    freq = 16.3516;
  ...
  // creating normalised LUT for Specone
  for (int i = 0; i < 256; ++i)
    table_specone[i] = static_cast<double>(i) / 255;
  ...
  for (unsigned int i = 0; i < image_frames; i++) {
    ...
    for (unsigned int y = 0; y < audio_frame_part; y++) {
      ...
      for (unsigned int x = 0; x < sine_bands; x++) {
        ...
        h = table_specone[ptr[index]];
        s = table_specone[ptr[index + 2]];
        l = table_specone[ptr[index + 1]];
...
void Specone::process(stk::StkFrames& audio, std::vector<cv::Mat>& images) {
